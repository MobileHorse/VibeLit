...
class NumberPicker extends StatelessWidget {
  ...
  NumberPicker.decimal(
      ...
      : assert(initialValue != null),
        ...
        selectedDecimalValue = ((initialValue - initialValue.floorToDouble()) * pow(10, decimalPlaces)).round(),
        intScrollController = new ScrollController(
          initialScrollOffset: (initialValue.floor() - minValue) * itemExtent,
        ),
        decimalScrollController = new ScrollController(
          initialScrollOffset: ((initialValue - initialValue.floorToDouble()) * pow(10, decimalPlaces)).roundToDouble() * itemExtent,
        ),
        super(key: key);
  ...
  animateIntfor(int valueToSelect, int duration) {
    _animate(intScrollController, (valueToSelect - minValue) * itemExtent, duration);
  }
  ...
  Widget _integerListView(ThemeData themeData) {
    TextStyle defaultStyle = defaultTextStyle ?? themeData.textTheme.body1;
    TextStyle selectedStyle = selectedTextStyle ?? themeData.textTheme.headline.copyWith(color: themeData.accentColor);
      ...
      child: new Container(
        ...
        child: new ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            final int value = minValue + index - 2;

            //define special style for selected (middle) element
            TextStyle itemStyle = value == selectedIntValue ? selectedStyle : defaultStyle;
            if (value == selectedIntValue - 2 || value == selectedIntValue + 2) {
              itemStyle = defaultStyle.copyWith(color: Colors.white.withOpacity(0.1));
            }

            bool isExtra = index == 0 || index == itemCount - 1 || index == 1 || index == itemCount - 2;

            return isExtra
                ? new Container(width: 20,) //empty first and last element
                : new Center(
  ...
  bool _onIntegerNotification(Notification notification) {
    if (notification is ScrollNotification) {
      //calculate
      int intIndexOfMiddleElement = (horizontal) ? (notification.metrics.pixels + listViewWidth / 2) ~/ itemExtent : (notification.metrics.pixels + listViewHeight / 2) ~/ itemExtent;
      int intValueInTheMiddle = minValue + intIndexOfMiddleElement - 2;
  ...
  bool _onDecimalNotification(Notification notification) {
    if (notification is ScrollNotification) {
      ...
      int indexOfMiddleElement = (horizontal) ? (notification.metrics.pixels + listViewWidth / 2) ~/ itemExtent : (notification.metrics.pixels + listViewHeight / 2) ~/ itemExtent;
  ...
  _animate(ScrollController scrollController, double value, [int duration = 1000]) {
    scrollController.animateTo(value, duration: new Duration(milliseconds: duration), curve: new ElasticOutCurve());
  }
}
...
class _NumberPickerDialogControllerState extends State<NumberPickerDialog> {
  ...
  NumberPicker _buildNumberPicker() {
    if (widget.decimalPlaces > 0) {
      return new NumberPicker.decimal(initialValue: selectedDoubleValue, minValue: widget.minValue, maxValue: widget.maxValue, decimalPlaces: widget.decimalPlaces, onChanged: _handleValueChanged);
    } else {
  ...
  Widget build(BuildContext context) {
    return new AlertDialog(
      ...
      actions: [
        ...
        new FlatButton(onPressed: () => Navigator.of(context).pop(widget.decimalPlaces > 0 ? selectedDoubleValue : selectedIntValue), child: widget.confirmWidget),
      ],
    );
  }
}
